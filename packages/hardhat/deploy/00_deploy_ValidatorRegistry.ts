...
const contractName = "ValidatorRegistry";
const deployValidatorRegistry: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  const namedAccounts = await hre.getNamedAccounts();
  const deployer = namedAccounts.deployer;
  
  let proxyAddress = getDeployArtifact(hre.network.name, contractName)?.address;
  const contractFactory = await hre.ethers.getContractFactory("core/ValidatorRegistry");
...
export default deployValidatorRegistry;

deployValidatorRegistry.tags = ["ValidatorRegistry", "core"]; 